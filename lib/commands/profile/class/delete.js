...
class Delete extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.getProfiles(profiles), names, 'class');
        }
        else {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.readFiles(directory), names, 'class');
        }
        if (username) {
