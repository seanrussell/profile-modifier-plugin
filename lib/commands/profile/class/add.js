...
class Add extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = await add_1.addToProfiles(directory, util_1.getProfiles(profiles), names, enabled, '', 'class');
        }
        else {
            profilesModified = await add_1.addToProfiles(directory, util_1.readFiles(directory), names, enabled, '', 'class');
        }
        if (username) {
