...
class Delete extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.getProfiles(profiles), names, 'field');
        }
        else {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.readFiles(directory), names, 'field');
        }
        if (username) {
