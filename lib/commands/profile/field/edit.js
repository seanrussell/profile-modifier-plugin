...
class Edit extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = edit_1.editInProfiles(directory, util_1.getProfiles(profiles), name, rename, false, permissions, 'field');
        }
        else {
            profilesModified = edit_1.editInProfiles(directory, util_1.readFiles(directory), name, rename, false, permissions, 'field');
        }
        if (username) {
