...
class Add extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = add_1.addToProfiles(directory, util_1.getProfiles(profiles), names, enabled, '', 'page');
        }
        else {
            profilesModified = add_1.addToProfiles(directory, util_1.readFiles(directory), names, enabled, '', 'page');
        }
        if (username) {
