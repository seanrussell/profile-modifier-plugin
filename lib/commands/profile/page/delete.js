...
class Delete extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.getProfiles(profiles), names, 'page');
        }
        else {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.readFiles(directory), names, 'page');
        }
        if (username) {
            ...
            this.ux.stopSpinner(`Pages removed from profiles and pushed to org ${username} successfully.`);
        }
        else {
            this.ux.stopSpinner('Pages removed from profiles successfully');
        }
        return {};
    }
}
