...
const messages = core_1.Messages.loadMessages('profile-additions', 'page');
class Edit extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = edit_1.editInProfiles(directory, util_1.getProfiles(profiles), name, rename, enabled, '', 'page');
        }
        else {
            profilesModified = edit_1.editInProfiles(directory, util_1.readFiles(directory), name, rename, enabled, '', 'page');
        }
        if (username) {
            ...
            this.ux.stopSpinner(`Pages edited in profiles and pushed to org ${username} successfully.`);
        }
        else {
            this.ux.stopSpinner('Pages edited in profiles successfully');
        }
        return {};
    }
}
