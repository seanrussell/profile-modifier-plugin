...
class Add extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = add_1.addToProfiles(directory, util_1.getProfiles(profiles), names, false, permissions, 'object');
        }
        else {
            profilesModified = add_1.addToProfiles(directory, util_1.readFiles(directory), names, false, permissions, 'object');
        }
        if (username) {
