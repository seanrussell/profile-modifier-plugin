...
class Delete extends command_1.SfdxCommand {
    async run() {
        ...
        if (profiles) {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.getProfiles(profiles), names, 'object');
        }
        else {
            profilesModified = delete_1.removeFromProfiles(directory, util_1.readFiles(directory), names, 'object');
        }
        if (username) {
