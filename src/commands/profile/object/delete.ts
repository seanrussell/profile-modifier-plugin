...
export default class Delete extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (profiles) {
      this.data = await removeFromProfiles(directories, getProfiles(profiles, this.project['path']), names, 'object');
    } else {
      this.data = await removeFromProfiles(directories, readFiles(directories), names, 'object');
    }
