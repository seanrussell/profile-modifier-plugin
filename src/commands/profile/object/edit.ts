...
export default class Edit extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    const filesModified = await editInProfiles(getFileNames(directories, profiles, this.project['path']), name, rename, false, permissions, 'object', alphabetize);

    this.ux.stopSpinner('Done');

    this.ux.styledHeader('Objects edited in profiles');
    this.ux.table(getDataForDisplay(filesModified, this.project['path'].length, 'edit', 'object'), ['Action', 'MetadataType', 'ProjectFile']);
