...
export default class Add extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (profiles) {
      this.data = await addToProfiles(directories, getProfiles(profiles, this.project['path']), names, enabled, '', 'class');
    } else {
      this.data = await addToProfiles(directories, readFiles(directories), names, enabled, '', 'class');
    }
