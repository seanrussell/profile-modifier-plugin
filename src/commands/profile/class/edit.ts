...
export default class Edit extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (profiles) {
      this.data = await editInProfiles(directories, getProfiles(profiles, this.project['path']), name, rename, enabled, '', 'class');
    } else {
      this.data = await editInProfiles(directories, readFiles(directories), name, rename, enabled, '', 'class');
    }
